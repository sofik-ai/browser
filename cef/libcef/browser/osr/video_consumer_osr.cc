...
void CefVideoConsumerOSR::OnFrameCaptured(
    ...
    mojo::PendingRemote<viz::mojom::FrameSinkVideoConsumerFrameCallbacks>
        callbacks) {
  // Sofik: for the accelerated path the frame is held until the next one
  // arrives (see held_frame_callbacks_), so Done() must NOT fire on return.
  // Every other path still releases immediately.
  std::optional<ScopedVideoFrameDone> scoped_done;
  if (!use_shared_texture_) {
    scoped_done.emplace(std::move(callbacks));
  }
  ...
  if (use_shared_texture_) {
    ...
    // Release the previous frame only now: its surface was live until this
    // one replaced it.
    held_frame_callbacks_.reset();
    held_frame_callbacks_.Bind(std::move(callbacks));
