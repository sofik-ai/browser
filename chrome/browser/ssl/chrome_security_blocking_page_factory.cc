...
#if !BUILDFLAG(ENABLE_CAPTIVE_PORTAL_DETECTION) && !BUILDFLAG(IS_ANDROID)
  // Sofik: desktop with captive portal detection compiled out.
  //
  // Upstream assumes !ENABLE_CAPTIVE_PORTAL_DETECTION means Android and
  // reaches for base::android and GetCaptivePortalServerUrl, so the file stops
  // compiling the moment the flag is turned off anywhere else. There is no
  // login page to open here: without the detector there is no captive portal
  // interstitial to press "connect" on.
  //
  // The flag is off because the detector probes a Google endpoint on network
  // changes -- traffic this browser has no reason to emit.
  (void)web_contents;
