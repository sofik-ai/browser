...
Browser* ChromeBrowserDelegate::CreateDevToolsBrowser(
  ...
  if (window_info.windowless_rendering_enabled) {
    // Upstream refuses, and Sofik measured what that costs and why the obvious
    // patch does not work.
    //
    // Refusing means falling through: the front end is built Chrome-style in a
    // window of its own. An embedder that asked for windowless rendering then
    // holds a browser it can never present -- CEF hands back an id, the host
    // allocates a texture, and no frame is ever painted into it. Measured on
    // 149.0.7827.201: begin frames arrive and the browser is sized, yet its
    // frame count stays at 0 while the page beside it runs at 60 Hz.
    //
    // Allowing Alloy style here instead only moves the failure: the call
    // arrives from Chrome's DevToolsWindow::CreateDevToolsBrowser, which
    // builds a Chrome Browser, and CreateDevToolsBrowser then aborts on
    // CHECK(platform_delegate->IsChromeStyle()). Off-screen DevTools needs the
    // front end created outside that path -- a plain Alloy OSR browser on the
    // DevTools URL, with the front end wired to the in-process CDP transport
    // -- not a flag flipped here.
    LOG(ERROR)
