...
display::ScreenInfos CefRenderWidgetHostViewOSR::GetNewScreenInfosForUpdate() {
  ...
  if (browser_impl_) {
    ...
    if (!handler->GetScreenInfo(browser_impl_.get(), screen_info) ||
      ...
      // Sofik: fall back to the real display, not to the view.
      //
      // Upstream uses the view rectangle here, which makes a page read a
      // monitor exactly the size of the browser it is inside: `screen.width ==
      // innerWidth`, `availHeight == height`, and 1x1 before any layout. No
      // real display behaves that way, and it costs a page one property read
      // to notice -- which is how this was found.
      //
      // The browser process already knows the actual screen through
      // display::Screen (NSScreen on macOS), so there is nothing to ask the
      // embedder for: work_area even excludes the menu bar, which an embedder
      // answering this can only approximate with a hardcoded inset.
      CefRect fallback;
      if (display::Screen::HasScreen()) {
        ...
        if (!bounds.IsEmpty()) {
          if (screen_info.rect.width == 0 || screen_info.rect.height == 0) {
            screen_info.rect = CefRect(bounds.x(), bounds.y(), bounds.width(),
                                       bounds.height());
          }
          if (screen_info.available_rect.width == 0 ||
              screen_info.available_rect.height == 0) {
            const gfx::Rect usable = work_area.IsEmpty() ? bounds : work_area;
      ...
      // Only if there is no screen at all -- a headless or service process --
      // does the view stand in, so popups still land somewhere sensible.
      if (screen_info.rect.width == 0 || screen_info.rect.height == 0 ||
          screen_info.available_rect.width == 0 ||
          screen_info.available_rect.height == 0) {
        CefRect screenRect;
