...
// TRACE_EVENT methods on Linux or iOS builds till after we set this up.
  ...
  // Sofik: the identity's timezone, seeded right after ICU comes up and
  // before anything reads a clock.
  //
  // Going through ICU rather than a later override is what keeps the readings
  // consistent: Intl.DateTimeFormat, Date::getTimezoneOffset() and the printed
  // zone name all resolve out of the same zone object, so they agree on the
  // offset *and* on the daylight-saving rule for the date. An override that
  // pins an offset is wrong for half the year, and three readings that
  // disagree is something a page checks in one line.
  //
  // Every process does this, not just the browser: a renderer resolving its
  // own zone from the machine would answer a different question than the
  // browser does.
  sofik::ApplyIdentityTimezone();
