...
bool DOMPluginArray::IsPdfViewerAvailable() {
  // Sofik: pinned to true.
  //
  // This one answer decides navigator.pdfViewerEnabled and whether
  // navigator.plugins and navigator.mimeTypes hold the five-plus-two entries
  // the HTML standard hard-codes or nothing at all. Upstream derives it from
  // the browser process having a plugin registered for application/pdf, which
  // is Chrome's PDF viewer -- an extension, with the extensions system under
  // it. This browser has neither, and an empty plugin list is the oldest
  // headless tell there is: every stock Chrome, on every platform, reports
  // the same fixed list.
  //
  // Only what script can read changes. No plugin is registered, so a PDF a
  // page navigates to is still a download, which is what an agent wants from
  // it anyway.
  //
  // A window that lost its frame has no plugin data upstream either.
  return window_ && window_->GetFrame();
}
