...
const blink::UserAgentBrandList GetUserAgentBrandList(
  ...
  DCHECK(parse_result);
  // Sofik: always claim the Google Chrome brand.
  //
  // A Chromium-branded build leaves `brand` empty here, so Sec-CH-UA and
  // navigator.userAgentData carry only "Chromium" and "Not)A;Brand" -- while
  // the UA string of the very same request says "Chrome/<n>". Two halves of
  // one request disagreeing is exactly what bot detection looks for, and it is
  // not fixable downstream: the CDP Network.setUserAgentOverride that corrects
  // the brands cannot land before the first request of a new browser.
  //
  // The brand comes from sofik::Identity rather than a literal here, so that
  // every surface a page can read about this machine has exactly one source.
  // version_info::GetProductName() answers "Chromium" in this build and is
  // therefore not usable.
  std::optional<std::string> brand = sofik::Identity::Get().brand();
