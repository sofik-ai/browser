// Sofik: window.chrome for the Sofik engine. Taken from
// chrome/renderer/loadtimes_extension_bindings.cc so that chrome.loadTimes()
// and chrome.csi() are the very same code a real Chrome runs, with chrome.app
// added; see sofik_chrome_object.h.
//
...
static const char* const kChromeObjectExtensionName = "v8/LoadTimes";

class ChromeObjectExtensionWrapper : public v8::Extension {
 public:
  ...
  // via these APIs.
  ChromeObjectExtensionWrapper() :
    v8::Extension(kChromeObjectExtensionName,
      "var chrome;"
      "if (!chrome)"
      "  chrome = {};"
      ...
      "  return GetCSI();"
      "};"
      // Sofik: chrome.app, which upstream gets from the extensions renderer.
      // The functions are native and carry Chrome's own names, so that
      // chrome.app.getDetails.toString() reads as it does there.
      "native function getDetails();"
      "native function getIsInstalled();"
      "native function installState();"
  ...
  v8::Local<v8::FunctionTemplate> GetNativeFunctionTemplate(
    ...
    } kAppFunctions[] = {{"getDetails", AppGetDetails},
                         {"getIsInstalled", AppGetIsInstalled},
                         {"installState", AppInstallState},
                         {"runningState", AppRunningState}};
    for (const auto& function : kAppFunctions) {
      if (name->StringEquals(
              v8::String::NewFromUtf8(isolate, function.name,
                                      v8::NewStringType::kInternalized)
                  .ToLocalChecked())) {
        return v8::FunctionTemplate::New(isolate, function.callback);
      }
    }
  ...
  // Sofik: what chrome.app answers on a page that is not an installed app,
  // which is every page.
  static void AppGetDetails(const v8::FunctionCallbackInfo<v8::Value>& args) {
    args.GetReturnValue().SetNull();
  }
  static void AppGetIsInstalled(
      const v8::FunctionCallbackInfo<v8::Value>& args) {
    args.GetReturnValue().Set(false);
  }
  static void AppRunningState(const v8::FunctionCallbackInfo<v8::Value>& args) {
    args.GetReturnValue().Set(
        v8::String::NewFromUtf8Literal(args.GetIsolate(), "cannot_run"));
  }
  static void AppInstallState(const v8::FunctionCallbackInfo<v8::Value>& args) {
    v8::Isolate* isolate = args.GetIsolate();
    if (args.Length() < 1 || !args[0]->IsFunction()) {
      return;
    }
    v8::Local<v8::Value> state =
        v8::String::NewFromUtf8Literal(isolate, "not_installed");
    // Failure here is the page's exception to see, not ours to handle.
    (void)args[0].As<v8::Function>()->Call(isolate->GetCurrentContext(),
                                           v8::Undefined(isolate), 1, &state);
  }
...
std::unique_ptr<v8::Extension> ChromeObjectExtension::Get() {
  return std::make_unique<ChromeObjectExtensionWrapper>();
}
