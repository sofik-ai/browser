...
def compile_proto(protoc_bin: str, protos_dir: Path, proto_file: Path) -> None:
  ...
  sh(protoc_bin, "--python_out=.", "--pyi_out=.", f"--proto_path={protos_dir}",
     proto_file)
...
def write_root_module(root_dir: Path) -> None:
  with (root_dir / "__init__.py").open("w", encoding="utf8") as root_module:
    ...
    all.sort()
    root_module.write("\n")
    root_module.write("__all__ = (\n")
    for import_name in all:
      root_module.write(f'    "{import_name}",\n')
    root_module.write(")\n")
...
def check_protoc_version(protoc_bin: str) -> None:
  ...
  pkg_version_parts = pkg_version_str.split(".")
  pkg_version = pkg_version_parts[1]
  if protoc_version != pkg_version:
    logging.debug("protoc version: %s, protobuf module version: %s",
                  protoc_version, pkg_version)
    raise ValueError(f"{protoc_bin!r}={protoc_version_str} and "
                     f"protobuf lib={pkg_version_str} versions must match.")
