...
bool Navigator::webdriver() const {
  // Sofik: pinned to false.
  //
  // The attribute itself is kept -- stock Chrome exposes navigator.webdriver
  // and returns false, so *removing* it would be the anomaly. What is severed
  // are the two paths that can flip it to true:
  //   1. RuntimeEnabledFeatures::AutomationControlledEnabled(), set by the
  //      --enable-automation command-line switch;
  //   2. probe::ApplyAutomationOverride(), the DevTools emulation agent's
  //      override -- reachable over CDP, which is exactly the channel our own
  //      automation runs on.
  //
  // Relying on "nobody happens to set either" made the false value incidental.
